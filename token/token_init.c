...
t_token	*ft_newtoken(char *cell, t_env **env, t_token_type type, int *i)
{
	t_token	*new;
		...
		|| type == C_BRACKET)
	{
		...
		ft_init_token_cmd(new, cell, env, i);
	return (new);
}
...
{
	if (line[*i] && line[*i] == '(')
		ft_tokenadd_back(&mini->tokens, ft_newtoken(line, &mini->env, O_BRACKET,
				i));
	else if (line[*i] && line[*i] == ')')
		ft_tokenadd_back(&mini->tokens, ft_newtoken(line, &mini->env, C_BRACKET,
				i));
	else if (line[*i] && line[*i] == '|' && line[*i + 1] == '|')
		ft_tokenadd_back(&mini->tokens, ft_newtoken(line, &mini->env, T_OR, i));
	else if (line[*i] && line[*i] == '|' && line[*i + 1] != '|')
		ft_tokenadd_back(&mini->tokens, ft_newtoken(line, &mini->env, T_PIPE,
				i));
	else if (line[*i] && line[*i] == '&' && line[*i + 1] == '&')
